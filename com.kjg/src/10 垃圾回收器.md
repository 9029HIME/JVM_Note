...
3.并发初始标记（init marking），此过程是三色标记算法，并且使用了STAB技术保证不丢失可达对象，具体过程如下：

​	3.1 在堆中的对象， 可以用3种颜色：黑、灰、白，初始阶段GC Roots是黑色，GC Roots直接引用的对象是灰色，其他的对象是白色。**有一个队列存放着所有的灰色对象**。

​	3.2 从灰色队列里拿出来一个灰色对象A，把它标记为黑色，将A对象

### Mixed GC（并行）：

**Mixed GC更特殊，它是根据G1在Old GC后，视情况决定是否进行。**
...
STAB：在并发标记阶段，如果用户线程将一个灰对象的属性设为null，那么这个属性原先指向的对象，也会认为它是可达的，虽然这样会产生浮动垃圾，但总比丢了要好。然后再STW，进行重新标记remark，将并发阶段认为的存活对象再检查一遍，去除浮动垃圾。

Mixed GC：先执行一次Young GC，和用户线程并发运行，采用三色标记法
