...
字节码指令中带#符号的就是符号引用，它用来指向某一个资源（变量、类、方法等）；

java代码运行时，会将字节码的常量池加载到方法区内，成为**运行时常量池**



每一个栈帧内部都包含着一个指向**运行时常量池**的**方法引用区**的**引用**，动态链接的过程是通过**引用**找到方法引用区，然后在方法引用区将符号引用转为直接引用。



在编译成字节码文件时，这个字节码所声明的变量、方法都会作为**符号引用**存在常量池里；**当自身或其他字节码需要使用这个变量、方法时，只需指向对应的符号引用即可**。符号引用最终会被动态链接称直接引用，通过直接引用寻找对应方法。
