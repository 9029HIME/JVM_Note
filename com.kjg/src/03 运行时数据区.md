...
字节码指令中带#符号的就是符号引用，它用来指向某一个资源（变量、类、方法等）。可以在字节码的常量池内通过符号引用找到对应的资源。每个字节码有自身的常量池。java代码运行时，会将字节码的常量池加载到方法区内，成为**运行时常量池**。每一个栈帧内部都包含着一个指向**运行时常量池**的**方法引用区**的**引用**，动态链接的过程是通过**引用**找到方法引用区，然后在方法引用区将符号引用转为直接引用。
